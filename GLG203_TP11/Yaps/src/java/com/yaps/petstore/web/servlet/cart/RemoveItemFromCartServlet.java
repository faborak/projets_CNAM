...
public class RemoveItemFromCartServlet extends AbstractServlet {
    ...
    protected void service(final HttpServletRequest request, final HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            delegate.removeItem(request.getParameter("itemId"));

            getServletContext().getRequestDispatcher("/viewcart").forward(request, response);

        } catch (RemoteException e) {
            Trace.throwing(getCname(), mname, e);
            getServletContext().getRequestDispatcher("/error.jsp?exception=Cannot remove an item to the shopping cart").forward(request, response);
        }
    }
}
