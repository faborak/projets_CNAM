...
public final class CustomerTest extends TestCase {
    ...
    public void testCreateCustomerWithInvalidValues() throws Exception {
        ...
        // Creates an object with invalid year of birth
        try {
        	final Customer customer = new Customer("bill000", "Bill", "Gates");
        	customer.setYearOfBirth(3000);
        	customer.checkData();
            fail("Customer cannot be set to invalid year of birth");
        } catch (CheckException e) {
        	assertEquals("Invalid customer year of birth", e.getMessage());
        }
    }
    ...
    public void testGetAge() {
    	...
    	// Creates a valid customer
    	final Customer customer = new Customer("bill000", "Bill", "Gates");
    	int yearOfBirth = 1950;
    	customer.setYearOfBirth(yearOfBirth);
    	assertTrue("Age should be greater then 50", customer.getAge() > 50);
    	int expectedAge = Customer.getCurrentYear() - yearOfBirth;
    	assertEquals(expectedAge, customer.getAge());
    	assertTrue("Customer should be adult", customer.isAdult());
    }
