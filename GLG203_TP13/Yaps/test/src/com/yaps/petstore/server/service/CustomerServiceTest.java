...
public final class CustomerServiceTest extends AbstractTestCase {
    ...
    /**
     * This test ensures that the method findCustomersWithNameStartingWith works. It does a first findCustomersWithNameLike, creates
     * a new object and does a second findCustomersWithNameStartingWith.
     */
    public void testDomainFindCustomersByName() throws Exception {
        final String id = getUniqueId();
        // First findAll
        int firstSize = 0;
        try {
        	firstSize = findNumberOfCustomersByName("Gat");
        }
        catch (ObjectNotFoundException e) { firstSize = 0;}

        // Create an object
        createCustomer(id, "Gateau", 1990);

        // Ensures that the object exists
        try {
            CustomerDTO dto = findCustomer(id);
            assertEquals("Gateau", dto.getLastname());
        } catch (ObjectNotFoundException e) {
            fail("Object has been created it should be found");
        }

        // Second findAll
        final int secondSize = findNumberOfCustomersByName("Gat");
    ...
    /**
     * This test ensures that the method findCustomersWithNameLike works. It does a first findCustomersWithNameLike, creates
     * a new object and does a second findCustomersWithNameLike.
     */
    public void testDomainFindCustomersByAge() throws Exception {
        final String id = getUniqueId();
        // First findAll
        int minAge = 25;
        int maxAge = 35;
        int firstSize = 0;
        try {
        	firstSize = findNumberOfCustomersByAge(minAge, maxAge);
        }
        catch (ObjectNotFoundException e) { firstSize = 0;}
        ...
        // Second findAll
        final int secondSize = findNumberOfCustomersByAge(minAge, maxAge);
   ...
   /**
    ...
    public void testServiceUpdateCustomer() throws Exception {
        final String id = getUniqueId();
        final String updatedId = getUniqueId();
    ...
    private int findNumberOfCustomersByName(String lastNameFirstChars) throws FinderException, RemoteException {
        try {
            return getCustomerService().findCustomersWithNameStartingWith(lastNameFirstChars).size();
        } catch (ObjectNotFoundException e) {
            return 0;
        }
    }

    private int findNumberOfCustomersByAge(int minAge, int maxAge) throws FinderException, RemoteException {
        try {
            return getCustomerService().findCustomersByAge(minAge, maxAge).size();
        } catch (ObjectNotFoundException e) {
            return 0;
        }
    }
    ...
    private void createCustomer(final String id, String lastName, int yearOfBirth) throws CreateException, CheckException, RemoteException {
        ...
        if ( yearOfBirth >= 0 )
        	customerDTO.setYearOfBirth(yearOfBirth);
    ...
    private String getUniqueId() throws RemoteException {
		return getCustomerService().getUniqueId();
	}
